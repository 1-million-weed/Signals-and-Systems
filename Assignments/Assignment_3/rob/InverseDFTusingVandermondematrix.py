...
class Inverse_DFT_using_Vandermonde_matrix():
    ...
    def calculate(self) -> None:
        """Calculate the inverse DFT using the Vandermonde matrix."""
        self.output_vector = []
        for row in range(self._length_signal):
            ...
            for column in range(self._length_signal):
                ...
                sum_real += cos_result * self._signal[column] * 1/self._length_signal
                sum_j += sin_result * self._signal[column] * 1/self._length_signal
...
if __name__ == "__main__":
    idvm = Inverse_DFT_using_Vandermonde_matrix()
    idvm.calculate()
    idvm.display()